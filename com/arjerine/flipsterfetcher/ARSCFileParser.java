 ...
 * Bodden, and others.
 ******************************************************************************/
package com.arjerine.flipsterfetcher;
...
public class ARSCFileParser extends AbstractResourceParser {
	...
	public static class ResType {
		...
		public ArrayList<AbstractResource> getAllResources() {
			...
			/*for (Map.Entry<String, AbstractResource> entry : resources.entrySet()) {
				Log.e("asd", entry.getKey());
			}*/
	...
	private void readResourceHeader(InputStream stream) throws IOException {
		...
		while (offset < remainingData.length - 1) {
			...
			if (nextChunkHeader.type == RES_STRING_POOL_TYPE) {
				System.out.println("res_string1");
				...
				System.out.println("res_string2");
				...
				System.out.println("res_string3");
						...
						for (int i = 0; i < typeTable.entryCount; i++) {
							...
							if (entry.flagsComplex) {
								...
								for (int j = 0; j < ((ResTable_Map_Entry) entry).count; j++) {
									...
									System.out.println(String.format("map-value1:%d-%s",map.name,parseValue(map.value)));
								}
							}
							else {
								...
								System.out.println(String.format("map-value2:%s-%d-%d-%d",res,val.data,val.dataType,val.res0));
							}
	...
	private int readStringTable
		...
		for (int i = 0; i < stringPoolHeader.stringCount; i++) {
			...
			if(DEBUG) {
				System.out.println(String.format("string@%d:%s",stringIdx,str));
			}
